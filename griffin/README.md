...
- **Army**: marine/marauder/medivac bio. Units get the `ATTACKING` role on
  creation. The bot rallies until it reaches an attack supply threshold,
  defends its bases if threatened, and uses ares `CombatManeuver` behaviors
  (stutter-step, health-aware retreat, influence-grid pathing) for per-unit
  micro. Bio stims when healthy and in weapons range of real targets;
